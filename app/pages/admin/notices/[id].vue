...
const { data: notice } = await useFetch<any>(API)
...
watchEffect(() => {
  if (notice.value) {
    ...
    form.coverImage = notice.value.coverImage
    form.publishedBy = notice.value.publishedBy
  }
})
...
<template>
  <div class="px-4 lg:px-8 py-8 max-w-4xl">
    <div class="mb-8 border-b border-gray-200 pb-8">
       ...
       <h1 class="text-3xl font-light text-gray-900 tracking-tighter">Edit Announcement</h1>
       <p class="text-gray-500 mt-2 font-light">Update content. Note: This will NOT resend emails.</p>
    </div>

    <div v-if="!notice" class="text-center py-20">
      Loading...
    </div>

    <form v-else @submit.prevent="submit" class="bg-white p-8 border border-gray-200 shadow-sm space-y-8">
       <div>
       ...
       <div>
         ...
         <textarea v-model="form.content" rows="12" required 
            class="w-full border border-gray-300 p-4 outline-none focus:border-black transition-colors bg-gray-50 focus:bg-white font-light text-gray-800"></textarea>
       </div>
