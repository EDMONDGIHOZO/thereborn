...
const stats = computed(() => [
    { title: 'Total Actors', value: statsData.value?.totalActors || 0, extra: 'Registered actors', icon: 'ri:user-star-line' },
    { title: 'Pending Approval', value: statsData.value?.pendingApprovals || 0, extra: 'Requires attention', icon: 'ri:time-line' },
    { title: 'Movies', value: statsData.value?.totalMovies || 0, extra: 'Across all profiles', icon: 'ri:movie-2-line' },
    { title: 'Engagement', value: statsData.value?.engagement || 0, extra: 'Profile views', icon: 'ri:bar-chart-groupped-line' }
])
...
<template>
  <div class="space-y-8 px-4 lg:px-8">
     ...
     <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-0 border border-gray-200 bg-white">
          <div v-for="stat in stats" :key="stat.title" class="p-6 border-b md:border-b-0 md:border-r border-gray-200 hover:bg-gray-50 transition-colors group">
               <h3 class="text-gray-500 text-sm uppercase tracking-wider mb-2 font-medium">{{ stat.title }}</h3>
               <div class="flex items-baseline justify-between">
                    <span class="text-4xl font-light text-gray-900 group-hover:text-blue-600 transition-colors">{{ stat.value }}</span>
          ...
          <div class="bg-gray-900 text-white p-8 flex flex-col">
               ...
               <div class="flex-1 space-y-6 overflow-y-auto pr-2 custom-scrollbar">
                    <div v-if="statsData?.recentActivities?.length">
                         <div v-for="(activity, i) in statsData.recentActivities" :key="i" class="border-b border-gray-700 pb-4 last:border-0 hover:pl-2 transition-all cursor-pointer">
